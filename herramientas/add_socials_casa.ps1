...
$socialIconsCasa = @"
                    <div class="flex flex-wrap gap-3 mt-6">
                        <a href="#" aria-label="Facebook" class="size-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 hover:bg-slate-800 hover:text-white transition-all shadow-sm">
                        ...
                        <a href="#" aria-label="Instagram" class="size-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 hover:bg-slate-800 hover:text-white transition-all shadow-sm">
                        ...
                        <a href="#" aria-label="X (Twitter)" class="size-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 hover:bg-slate-800 hover:text-white transition-all shadow-sm">
                        ...
                        <a href="#" aria-label="YouTube" class="size-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 hover:bg-slate-800 hover:text-white transition-all shadow-sm">
                        ...
                        <a href="#" aria-label="Flickr" class="size-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 hover:bg-slate-800 hover:text-white transition-all shadow-sm group">
...
foreach ($file in $casaFiles) {
    ...
    if (-not ($content -match 'aria-label="Facebook"')) {
        ...
        # In casa-historica footers, the branding is designated by <!-- Branding --> comment as well
        if ($content -match '<!-- Branding -->') {
            $content = (($content -split '<!-- Branding -->')[0]) + '<!-- Branding -->' + (($content -split '<!-- Branding -->')[1] -replace $patternToReplace, $socialIconsCasa)
            [System.IO.File]::WriteAllText($file.FullName, $content, [System.Text.Encoding]::UTF8)
            Write-Host "Updated Casa Socials in $($file.Name)"
        }
    }
}
